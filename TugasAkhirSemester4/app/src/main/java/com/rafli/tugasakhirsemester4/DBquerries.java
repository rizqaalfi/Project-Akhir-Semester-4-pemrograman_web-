...
public class DBquerries {
    ...
    public static List<List<DevitaCollectionModel>> lists=new ArrayList<>();
    ...
    public static List<AdressesModel> addressesModelList=new ArrayList<>();
                        ...
                        if(task.isSuccessful()){
                            for(QueryDocumentSnapshot queryDocumentSnapshot : task.getResult()){
                                ...
                                if((long)queryDocumentSnapshot.get("view_type") == 0){
                                    ...
                                    lists.get(index).add(new DevitaCollectionModel(0,sliderModelList));

                                }

                                else if((long)queryDocumentSnapshot.get("view_type") == 1){

                                    lists.get(index).add(new DevitaCollectionModel(1,queryDocumentSnapshot.get("strip_ad_banner").toString(),queryDocumentSnapshot.get("background").toString()));
                                    ...
                                    lists.get(index).add(new DevitaCollectionModel(2,queryDocumentSnapshot.get("layout_title").toString(),queryDocumentSnapshot.get("layout_background").toString(),horizontalProductScrollModelList,viewAllProductList));
                                    ...
                                    lists.get(index).add(new DevitaCollectionModel(3,queryDocumentSnapshot.get("layout_title").toString(),queryDocumentSnapshot.get("layout_background").toString(),gridProductScrollModelList));

                                }
                            }
                            DevitaCollectionAdapter DevitaCollectionAdapter=new DevitaCollectionAdapter(lists.get(index));
                            DevitaCollectionRecyclerView.setAdapter(DevitaCollectionAdapter);
                            DevitaCollectionAdapter.notifyDataSetChanged();
                            DevitaCollectionFragment.swipeRefreshLayout.setRefreshing(false);
                        }
                        else {
                            String error=task.getException().getMessage();
                            Toast.makeText(context,error,Toast.LENGTH_SHORT).show();
